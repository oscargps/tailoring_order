...
import { Button, Checkbox, Input, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, getKeyValue } from "@nextui-org/react";
...
const UpdateOrderModal = (props: Props) => {
    const { actualElements, isOpen, onOpenChange, selectedElements, onSubmit } = props;
    ...
    const [elementsUpdated, setElementsUpdated] = useState<Record<string, number | undefined>>({})
    ...
    const mapElements = useCallback(() => {
        ...
        return selectedElements.map((element: string) => {
            const selectedElement = actualElements.find((selectedEl) => selectedEl.id == parseInt(element))
            if (selectedElement) {
                const elementId = selectedElement?.id
                const isTotalElement = totalElements.includes(elementId.toString())
                const model = Models ? Models.find(model => model.id === selectedElement?.model_id)?.model_name : selectedElement?.model_id
                const size = Literals ? Literals.find(literal => literal.id === selectedElement?.element_size)?.literal_name : selectedElement?.element_size
                const gender = Literals ? Literals.find(literal => literal.id === selectedElement?.element_gender)?.literal_name : selectedElement?.element_gender

                return {
                    id: selectedElement?.id,
                    element: `${model}/${gender}/T:${size}/${selectedElement?.element_quantity}`,
                    new_quantity: (<Input className="m-4 w-4/6 md:w-2/6"
                        size="sm"
                        type="number"
                        isRequired
                        isInvalid={(elementsUpdated[elementId] || 0) > selectedElement?.element_quantity}
                        inputMode="numeric"
                        pattern="[0-9\s]{13,19}"
                    ...
                    />),
                    check: (<Checkbox name={(selectedElement?.id)?.toString()} isSelected={totalElements.includes((selectedElement?.id)?.toString())} onChange={addTotalElements}></Checkbox>)
                }
            }
        })
    ...
    const addTotalElements = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const totalElementId = actualTotalElements.findIndex(el => el == selectedElement)
        if (totalElementId >= 0) {
            ...
            setElementsUpdated({
                ...elementsUpdated,
                [selectedElement]: actualElements.find((selectedEl) => selectedEl.id == parseInt(selectedElement))?.element_quantity
            })
        }
    ...
    return (
        <>
            <Modal isOpen={isOpen} size={'5xl'} onOpenChange={onOpenChange} placement="top">
                <ModalContent>
                    {(onClose) => (
                        <>
                            ...
                            <ModalFooter>
                                ...
                                <Button color="success" onPress={() => { onSubmit(elementsUpdated) }}>
                                    Confirmar
                                </Button>
                            </ModalFooter>
                        </>
